...
export function containsSubstantialBootEcho(
  ...
  for (let i = 0; i <= needle.length - minLen; i += ECHO_SCAN_STEP) {
    const chunk = needle.slice(i, i + minLen);
    if (haystack.includes(chunk)) {
      return true;
    }
  }
  // Always check the final window so we don't miss a chunk that lands
  // on the tail boundary outside the step grid.
  const tailChunk = needle.slice(needle.length - minLen);
  if (tailChunk.length >= minLen && haystack.includes(tailChunk)) {
    return true;
  }
  return false;
}
